...
const RemovalTech = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/all_removal_info`);
      if (!res.ok) {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: 'eng_no', //normal accessorKey
        header: 'Engine #',
        size: 100,
      },
      {
    ...
    <div className='flex h-[100vh] bg-black'>
      ...
      <div className=' rounded-xl m-2 p-2 w-100 overflow-y-scroll ' style={{ backgroundColor: "#F0F0F0" }}>
        ...
        <div className=' bg-white mt-4'>
          <h1 className='text-xl font-semibold bg-black text-white p-2 '>Removal Information</h1>
