...
const MISITech = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/all_mis_info`);
      if (!res.ok) {
  ...
  const columns = useMemo(
    () => [
      {
        accessorKey: 'segment', //access nested data with dot notation
        header: 'Segment',
        size: 100,
      },
      {
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        {/* <h1 className='font-semibold'>MISI Information</h1> */}
        <IconButton
          onClick={() => {
            handleExportData();
          }}
    ...
    <div className='flex h-[100vh] bg-black'>
      ...
      <div className=' rounded-xl m-2 p-2 w-100 overflow-y-scroll ' style={{ backgroundColor: "#F0F0F0" }}>
        ...
        <div className=' bg-white mt-4'>
          <h1 className='text-xl font-semibold bg-black text-white p-2 '>MISI Information</h1>
